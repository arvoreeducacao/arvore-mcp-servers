...
function server() {
  return new GoogleDocsMCPServer({
    client: CLIENT,
    transport: "stdio",
    host: "127.0.0.1",
    port: 0,
...
describe("GoogleDocsMCPServer", () => {
  it("builds with a service identity", () => {
    expect(server()).toBeInstanceOf(GoogleDocsMCPServer);
  });

  it("builds without one, for OAuth-only deployments", () => {
    expect(
      new GoogleDocsMCPServer({
        client: null,
        transport: "http",
        host: "127.0.0.1",
        port: 0,
    ...
    ).toBeInstanceOf(GoogleDocsMCPServer);
  });
});
...
describe("OAuthProvider routing", () => {
  const oauth = new OAuthProvider({
    ...
    issuer: () => "https://docs-mcp.example.com",
  });
