...
import { GoogleDocsMCPServer } from "./server.js";
...
const clientId = process.env.GDOCS_MCP_CLIENT_ID || process.env.GDRIVE_MCP_CLIENT_ID;
const clientSecret =
  process.env.GDOCS_MCP_CLIENT_SECRET || process.env.GDRIVE_MCP_CLIENT_SECRET;
const refreshToken = process.env.GDOCS_MCP_REFRESH_TOKEN;

if (!clientId || !clientSecret) {
  console.error(
    "Error: GDOCS_MCP_CLIENT_ID and GDOCS_MCP_CLIENT_SECRET are required.\n" +
      "Create an OAuth Desktop client at https://console.cloud.google.com/apis/credentials,\n" +
      "enable the Google Docs API, then run `google-docs-mcp auth login`."
  );
  process.exit(1);
}

const hasSignIn = !!(
  process.env.GDOCS_MCP_SIGNIN_CLIENT_ID && process.env.GDOCS_MCP_SIGNIN_CLIENT_SECRET
);

if (!refreshToken && !hasSignIn) {
  console.error(
    "Error: GDOCS_MCP_REFRESH_TOKEN is required unless Google sign-in is configured.\n" +
      "Run `google-docs-mcp auth login` locally and store the printed refresh token,\n" +
      "or set GDOCS_MCP_SIGNIN_CLIENT_ID/SECRET so each user connects with their own account."
  );
  process.exit(1);
}
...
if (transport === "http") {
  ...
  if (!signingKey) {
    ...
    process.stderr.write(
      "[google-docs-mcp] MCP_TOKEN_SIGNING_KEY is not set — generated an ephemeral one.\n" +
...
const signInClientId = process.env.GDOCS_MCP_SIGNIN_CLIENT_ID;
const signInClientSecret = process.env.GDOCS_MCP_SIGNIN_CLIENT_SECRET;
const signInDomains = (process.env.GDOCS_MCP_SIGNIN_DOMAINS || "")
  .split(",")
  .map((domain) => domain.trim().toLowerCase().replace(/^@/, ""))
  .filter(Boolean);
const signInScopes = (process.env.GDOCS_MCP_SIGNIN_SCOPES || DEFAULT_SCOPES.join(","))
  .split(",")
...
if ((signInClientId || signInClientSecret) && !(signInClientId && signInClientSecret)) {
  console.error(
    "Error: GDOCS_MCP_SIGNIN_CLIENT_ID and GDOCS_MCP_SIGNIN_CLIENT_SECRET must be set together."
  );
  process.exit(1);
}
...
if (signInClientId && signInDomains.length === 0) {
  console.error(
    "Error: GDOCS_MCP_SIGNIN_DOMAINS is required when Google sign-in is enabled.\n" +
      "List the email domains allowed to authorize a client, e.g. GDOCS_MCP_SIGNIN_DOMAINS=example.com"
  );
  process.exit(1);
}
...
try {
  ...
  console.error("Failed to start Google Docs MCP Server:", error);
  process.exit(1);
}
...
async function runAuthCommand(args: string[]): Promise<void> {
  ...
  const authClientId =
    process.env.GDOCS_MCP_CLIENT_ID || process.env.GDRIVE_MCP_CLIENT_ID;
  ...
  if (action === "login") {
    if (!authClientId || !authClientSecret) {
      console.error(
        "Error: GDOCS_MCP_CLIENT_ID and GDOCS_MCP_CLIENT_SECRET must be set before `auth login`."
      );
      process.exit(1);
    }
    ...
    const credentials = await runAuthorizationFlow({
      ...
      port: process.env.GDOCS_MCP_REDIRECT_PORT
        ? parsePort(process.env.GDOCS_MCP_REDIRECT_PORT, 0)
        : undefined,
      loginHint: process.env.GDOCS_MCP_LOGIN_HINT,
    });

    console.error("\nAuthorized. Store this as GDOCS_MCP_REFRESH_TOKEN:\n");
  ...
  if (action === "logout") {
    const token = process.env.GDOCS_MCP_REFRESH_TOKEN;
    if (!token) {
      console.error("Nothing to revoke: GDOCS_MCP_REFRESH_TOKEN is not set.");
      process.exit(1);
    }
  ...
  console.error(
    `Unknown auth subcommand: ${action}\nUsage: google-docs-mcp auth <login|logout>`
  );
  process.exit(1);
}
