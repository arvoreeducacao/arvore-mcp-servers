...
const clientId = process.env.GSHEETS_MCP_CLIENT_ID || process.env.GDRIVE_MCP_CLIENT_ID;
const clientSecret =
  process.env.GSHEETS_MCP_CLIENT_SECRET || process.env.GDRIVE_MCP_CLIENT_SECRET;
const refreshToken = process.env.GSHEETS_MCP_REFRESH_TOKEN;

if (!clientId || !clientSecret) {
  console.error(
    "Error: GSHEETS_MCP_CLIENT_ID and GSHEETS_MCP_CLIENT_SECRET are required.\n" +
      "Create an OAuth Desktop client at https://console.cloud.google.com/apis/credentials,\n" +
      "enable the Google Sheets API, then run `google-sheets-mcp auth login`."
  );
  process.exit(1);
}

const hasSignIn = !!(
  process.env.GSHEETS_MCP_SIGNIN_CLIENT_ID && process.env.GSHEETS_MCP_SIGNIN_CLIENT_SECRET
);

if (!refreshToken && !hasSignIn) {
  console.error(
    "Error: GSHEETS_MCP_REFRESH_TOKEN is required unless Google sign-in is configured.\n" +
      "Run `google-sheets-mcp auth login` locally and store the printed refresh token,\n" +
      "or set GSHEETS_MCP_SIGNIN_CLIENT_ID/SECRET so each user connects with their own account."
  );
  process.exit(1);
}
...
if (transport === "http") {
  ...
  if (!signingKey) {
    ...
    process.stderr.write(
      "[google-sheets-mcp] MCP_TOKEN_SIGNING_KEY is not set — generated an ephemeral one.\n" +
...
const signInClientId = process.env.GSHEETS_MCP_SIGNIN_CLIENT_ID;
const signInClientSecret = process.env.GSHEETS_MCP_SIGNIN_CLIENT_SECRET;
const signInDomains = (process.env.GSHEETS_MCP_SIGNIN_DOMAINS || "")
  .split(",")
  .map((domain) => domain.trim().toLowerCase().replace(/^@/, ""))
  .filter(Boolean);
const signInScopes = (process.env.GSHEETS_MCP_SIGNIN_SCOPES || DEFAULT_SCOPES.join(","))
  .split(",")
...
if ((signInClientId || signInClientSecret) && !(signInClientId && signInClientSecret)) {
  console.error(
    "Error: GSHEETS_MCP_SIGNIN_CLIENT_ID and GSHEETS_MCP_SIGNIN_CLIENT_SECRET must be set together."
  );
  process.exit(1);
}
...
if (signInClientId && signInDomains.length === 0) {
  console.error(
    "Error: GSHEETS_MCP_SIGNIN_DOMAINS is required when Google sign-in is enabled.\n" +
      "List the email domains allowed to authorize a client, e.g. GSHEETS_MCP_SIGNIN_DOMAINS=example.com"
  );
  process.exit(1);
}
...
try {
  ...
  console.error("Failed to start Google Sheets MCP Server:", error);
  process.exit(1);
}
...
async function runAuthCommand(args: string[]): Promise<void> {
  ...
  const authClientId =
    process.env.GSHEETS_MCP_CLIENT_ID || process.env.GDRIVE_MCP_CLIENT_ID;
  ...
  if (action === "login") {
    if (!authClientId || !authClientSecret) {
      console.error(
        "Error: GSHEETS_MCP_CLIENT_ID and GSHEETS_MCP_CLIENT_SECRET must be set before `auth login`."
      );
      process.exit(1);
    }
    ...
    const credentials = await runAuthorizationFlow({
      ...
      port: process.env.GSHEETS_MCP_REDIRECT_PORT
        ? parsePort(process.env.GSHEETS_MCP_REDIRECT_PORT, 0)
        : undefined,
      loginHint: process.env.GSHEETS_MCP_LOGIN_HINT,
    });

    console.error("\nAuthorized. Store this as GSHEETS_MCP_REFRESH_TOKEN:\n");
  ...
  if (action === "logout") {
    const token = process.env.GSHEETS_MCP_REFRESH_TOKEN;
    if (!token) {
      console.error("Nothing to revoke: GSHEETS_MCP_REFRESH_TOKEN is not set.");
      process.exit(1);
    }
  ...
  console.error(
    `Unknown auth subcommand: ${action}\nUsage: google-sheets-mcp auth <login|logout>`
  );
  process.exit(1);
}
