  ...
  return new Promise((resolve, reject) => {
    const server = createServer((req: IncomingMessage, res: ServerResponse) => {
      ...
      if (error) {
        ...
        server.close();
        reject(new GoogleDriveMCPError(`OAuth error: ${error}`, "AUTH_ERROR"));
        return;
      }
      ...
      if (state !== opts.expectedState) {
        ...
        server.close();
        reject(
          new GoogleDriveMCPError("OAuth state mismatch — aborting", "AUTH_ERROR")
        );
        return;
      }
      ...
      if (!code) {
        ...
        server.close();
        reject(new GoogleDriveMCPError("Missing OAuth code", "AUTH_ERROR"));
        return;
      }
...
export async function exchangeCodeForToken(params: {
  ...
  if (!tokens.refresh_token) {
    throw new GoogleDriveMCPError(
      "No refresh_token returned. Run `google-drive-mcp auth logout` to revoke and try again.",
      "AUTH_ERROR"
    );
  }
