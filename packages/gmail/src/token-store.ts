...
import { OAuthCredentials, GmailMCPError } from "./types.js";

const KEYCHAIN_SERVICE = "arvoretech-gmail-mcp";
...
export class TokenStore {
  ...
  constructor(configDir?: string) {
    this.configDir =
      configDir ||
      process.env.GMAIL_MCP_CONFIG_DIR ||
      join(homedir(), ".config", "arvoretech-gmail-mcp");
  ...
  async load(): Promise<OAuthCredentials | null> {
    ...
    if (!key) {
      throw new GmailMCPError(
        "Encryption key missing — credentials file exists but key cannot be loaded. Re-run `gmail-mcp auth login` to recreate.",
        "AUTH_ERROR"
      );
    }
...
export function deriveTransientKey(seed: string): Buffer {
